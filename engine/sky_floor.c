...
/*   Updated: 2020/07/28 16:25:09 by youlee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			draw_floor(t_cub *cub, t_object *obj, t_pos *pixel,
		t_pos *tex_pos)
{
	t_texture	*tex;
	...
	if (!tex->tex)
		coord(&cub->window, pixel, cal_color(cub->c[TEX_FLOOR],
					cub->cam_height[obj->row] / 3.5));
	else
	{
		...
		coord(&cub->window, pixel, cal_color(cal_color2(tex, tex_pos),
				(cub->window.half.y * 2.) / ((2. * (double)obj->row) -
					cub->window.half.y * 2.) / 3.5));
	}
}

static void			draw_sky(t_cub *cub, t_object *obj,
		t_pos *pixel, t_pos *tex_pos)
{
	t_texture	*tex;
...
void				draw_sky_floor(t_cub *cub, t_object *obj)
{
	...
	{
		obj->row = (int)i;
		w = cub->window.size.y / ((2. * (double)i) -
				(cub->window.half.y * 2.)) / obj->dist;
