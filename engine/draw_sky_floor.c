...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/07/01 22:05:01 by youlee            #+#    #+#             */
/*   Updated: 2020/07/01 22:24:41 by youlee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "engine.h"

static void		init_draw_sky_floor(t_raysult *ray)
{
...
static void		draw_sky_pixel(t_game *game, t_raysult *ray, t_pos *pixel, t_pos *p_tex)
{
	t_tex		*tex;
	...
	if (!game->tex[TEX_SKY].tex)
	{
		draw_pixel(&game->window, pixel,
				dist_shade(game->options, game->config.c[TEX_SKY],
	...
	else
	{
		...
		draw_pixel(&game->window, pixel,
				dist_shade(game->options, get_tex_color(tex, p_tex),
					game->sf_dist[ray->row]));
	}
}

static void		draw_floor_pixel(t_game *game, t_raysult *ray, t_pos *pixel, t_pos *p_tex)
{
	t_tex		*tex;
	...
	{
		draw_pixel(&game->window, pixel,
				dist_shade(game->options, game->config.c[TEX_FLOOR],
					game->sf_dist[ray->row]));
	}
	else
	{
...
void			draw_sky_floor(t_game *game, t_raysult *ray)
{
	...
	{
		ray->row = i;
		weight = game->sf_dist[i] / ray->dist;
		set_pos(&ray->c_floor,
